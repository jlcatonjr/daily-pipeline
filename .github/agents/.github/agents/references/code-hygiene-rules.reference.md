...
# Code Hygiene Rules — Enforcement Catalog (MusicMaker)
...
**Enforcement check:**
```
find src/ -name "*.backup" -o -name "*.bak" -o -name "*_old.*" \
...
### CH-03 — No Ad-Hoc Scripts in `src/`

Source directories contain only production code. Investigative, debug, fix, and benchmark scripts must not reside in `src/`.
