# Infrastructure Security Layers — daily-pipeline
...
- **Scope gate.** This reference applies only to projects that **build, deploy, run, or operate a
  program, server, or service**. If daily-pipeline ships no deployed software (e.g. a pure
...
- **Honest posture language:** record "engages as tested on daily-pipeline's target," never
...
<!-- Project-owned: record daily-pipeline's chosen layers, tools, deviations, and rationale below.
